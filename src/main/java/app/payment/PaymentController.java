...
public class PaymentController {
    public static Route createPaymentIntent = (Request request, Response response) -> {
        ...
        System.err.println("Json Received: " + basket.toString());

        Payment payment = new Payment(basket);
        System.err.println("App Step: " + payment);
        response.status(200);
        response.type("application/json");
        String pubKeyString = System.getenv("STRIPE_PUBLISHABLE_KEY");

        Object pubKeyObject = pubKeyString;
        Map<String, Object> wrapper = new HashMap<String, Object>();
        wrapper.put("paymentIntent", payment.getPaymentIntent());
        wrapper.put("publishable_Key", pubKeyObject);
        wrapper.put("Added wrapper", "This is added to wrapper");

        String testJSON = ApiResource.GSON.toJson(wrapper);
        System.err.println(testJSON);
